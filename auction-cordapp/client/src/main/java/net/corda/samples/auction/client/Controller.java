...
public class Controller {
    ...
    @PostMapping("createIou")
    public APIResponse<Void> createIou(@RequestBody Forms.SettlementForm settlementForm){
        double numericValue = Double.parseDouble(Amount.parseCurrency(settlementForm.getAmount()).toString().split(" ")[0]);
        int amount = (int) numericValue;
        Party otherParty = settlementForm.getParty();

        System.out.println("----------------inside controller --------------------------------------------------");
        System.out.println(amount);
        System.out.println(otherParty);
        try {
            activeParty.startFlowDynamic(Initiator.class,
                            amount,
                            otherParty)
                    .getReturnValue().get();
            return APIResponse.success();
